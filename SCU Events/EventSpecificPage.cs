	...
	public class EventSpecificPage : ContentPage
	{
		...
		public EventSpecificPage(EventItem item)
		{
			...
			for (int i = 0; i < app.MyEvents_collection.Count; i++)
			{
				...
				{
					my_index = i;
					app.AllEvents_collection[app.all_index].IsFavorited = true;
					break;
				}
				else 
			...
			desc = new Label
			{
				Text = "Date: " + item.Date.ToString("d") + "\nTime: " + item.Time + "\nLocation: " + item.Location + "\n\n" + item.Info,
				HorizontalTextAlignment = TextAlignment.Center,
		...
		void buttonClicked(object sender, EventArgs e)
		{
			if (sender == add_to_events)
			{
				...
				app.MyEvents_collection.Add(app.AllEvents_collection[app.all_index]);
		...
		void setAddRemove()
		{
			if (app.AllEvents_collection[app.all_index].IsFavorited == false)
			{
